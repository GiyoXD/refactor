...
def write_header_new(
    worksheet: Worksheet,
    start_row: int,
    header_cell_configs: List[List[Optional[Dict[str, Any]]]],
    sheet_styling_config: Optional[Dict[str, Any]] = None
) -> Optional[Dict[str, Any]]:
    """
    Writes a header to the worksheet based on a structured cell configuration.
    This version uses an occupancy grid to handle complex row/colspans.

    Args:
        worksheet: The openpyxl Worksheet object.
        start_row: The 1-based row index where the header should start.
        header_cell_configs: A list of lists, where each inner list represents a row.
                             Each item in the inner list is a dictionary defining a cell,
                             or None for a placeholder cell (spanned by a previous cell).
                             Cell dictionary can contain:
                               - "value": The text/content of the cell.
                               - "id": A unique identifier for this header cell, used for column_map.
                               - "rowspan": (Optional) Number of rows this cell spans (default 1).
                               - "colspan": (Optional) Number of columns this cell spans (default 1).
                               - "font": (Optional) Specific font config for this cell (Openpyxl Font kwargs).
                               - "alignment": (Optional) Specific alignment for this cell (Openpyxl Alignment kwargs).
                               - "fill": (Optional) Specific fill for this cell (Openpyxl PatternFill kwargs).
        sheet_styling_config: Optional dictionary for overall header styling
                              (e.g., header_font, header_alignment, header_pattern_fill).

    Returns:
        A dictionary containing header info:
        {'first_row_index': int, 'last_header_row': int, 'column_map': Dict[str, int], 'num_columns': int}
        or None if header_cell_configs is invalid.
    """
    if not header_cell_configs or not isinstance(header_cell_configs, list) or \
       not all(isinstance(r, list) for r in header_cell_configs):
        # print("Error: Invalid header_cell_configs format.")
        return None
    if start_row <= 0:
        # print("Error: Invalid start_row.")
        return None

    num_header_rows = len(header_cell_configs)
    if num_header_rows == 0:
        # print("Error: header_cell_configs is empty.")
        return None

    # Determine the maximum number of logical columns based on config structure
    # This is tricky because of colspans. We need to simulate placement.
    # A simpler way for now: find the row with the most non-None items + their colspans.
    max_logical_cols = 0
    for r_cfg in header_cell_configs:
        current_row_logical_cols = 0
        for cell_cfg in r_cfg:
            if cell_cfg and isinstance(cell_cfg, dict):
                current_row_logical_cols += cell_cfg.get("colspan", 1)
            elif cell_cfg is None: # Placeholder, counts as 1 logical column in its row
                current_row_logical_cols += 1
        max_logical_cols = max(max_logical_cols, current_row_logical_cols)

    if max_logical_cols == 0: # Fallback if all rows are empty or only contain Nones
        max_logical_cols = max(len(r) for r in header_cell_configs) if header_cell_configs else 0
        if max_logical_cols == 0:
            # print("Error: Could not determine number of columns for header.")
            return None


    occupancy_grid = [[False for _ in range(max_logical_cols)] for _ in range(num_header_rows)]
    final_column_map: Dict[str, int] = {}
    last_header_row = start_row + num_header_rows - 1

    # print(f"Debug: Unmerging header block for write_header_new: R{start_row}-R{last_header_row}, C1-C{max_logical_cols}")
    unmerge_block(worksheet, start_row, last_header_row, max_logical_cols)

    # --- Determine Header Styling from sheet_styling_config ---
    header_font_to_apply = bold_font
    header_alignment_to_apply = center_alignment
    header_border_to_apply = thin_border
    header_background_fill_to_apply = None
    ...
    if sheet_styling_config:
        ...
        if header_font_cfg and isinstance(header_font_cfg, dict):
            try: header_font_to_apply = Font(**{k: v for k, v in header_font_cfg.items() if v is not None})
            except TypeError: pass

        header_align_cfg = sheet_styling_config.get("header_alignment")
        if header_align_cfg and isinstance(header_align_cfg, dict):
            try: header_alignment_to_apply = Alignment(**{k: v for k, v in header_align_cfg.items() if v is not None})
            except TypeError: pass
        
        header_fill_cfg = sheet_styling_config.get("header_pattern_fill")
        if header_fill_cfg and isinstance(header_fill_cfg, dict):
            params = {k: v for k, v in header_fill_cfg.items() if v is not None}
            if "start_color" in params and "fgColor" not in params: params["fgColor"] = params.pop("start_color")
            if "end_color" in params and "bgColor" not in params: params["bgColor"] = params.pop("end_color")
            if "fill_type" in params and "patternType" not in params: params["patternType"] = params.pop("fill_type")
            if params.get("fgColor") and params.get("patternType"):
                try: header_background_fill_to_apply = PatternFill(**params)
                except TypeError: pass

    # --- Iterate through header_cell_configs to write cells and apply merges ---
    for r_offset, row_config in enumerate(header_cell_configs):
        current_ws_row = start_row + r_offset
        # This cursor tracks the current *physical* column in the worksheet we are trying to write to.
        physical_col_idx = 1 

        for logical_c_offset, cell_config_item in enumerate(row_config):
            # Find the next available physical column in the worksheet for this logical cell
            # by skipping already occupied cells (due to previous colspans/rowspans).
            while physical_col_idx <= max_logical_cols and \
                  (r_offset < num_header_rows and occupancy_grid[r_offset][physical_col_idx -1]):
                physical_col_idx += 1
            
            if physical_col_idx > max_logical_cols:
                # This means the row definition is wider than max_logical_cols, or occupancy grid logic is off.
                # print(f"Warning: Physical column index {physical_col_idx} exceeded max_logical_cols {max_logical_cols} at row_offset {r_offset}, logical_c_offset {logical_c_offset}. Skipping rest of row.")
                break 

            if cell_config_item is None: # This is a placeholder for a cell spanned by a previous definition
                # Mark as occupied if not already (should be, but defensive)
                if not occupancy_grid[r_offset][physical_col_idx -1]:
                     occupancy_grid[r_offset][physical_col_idx -1] = True
                physical_col_idx +=1 # Move to the next physical column for the next logical cell
                continue

            if not isinstance(cell_config_item, dict):
                # print(f"Warning: Invalid cell_config_item at row_offset {r_offset}, logical_c_offset {logical_c_offset}. Expected dict, got {type(cell_config_item)}. Skipping.")
                physical_col_idx +=1
                continue
            
            value = cell_config_item.get("value")
            cell_id = cell_config_item.get("id")
            rowspan = cell_config_item.get("rowspan", 1)
            colspan = cell_config_item.get("colspan", 1)

            # Cell-specific styles (optional, override header defaults)
            cell_font_override = cell_config_item.get("font")
            cell_alignment_override = cell_config_item.get("alignment")
            cell_fill_override = cell_config_item.get("fill")

            actual_cell = worksheet.cell(row=current_ws_row, column=physical_col_idx)
            actual_cell.value = value

            final_font = header_font_to_apply
            if cell_font_override and isinstance(cell_font_override, dict):
                try: final_font = Font(**{k:v for k,v in cell_font_override.items() if v is not None})
                except TypeError: pass
            actual_cell.font = final_font

            final_alignment = header_alignment_to_apply
            if cell_alignment_override and isinstance(cell_alignment_override, dict):
                try: final_alignment = Alignment(**{k:v for k,v in cell_alignment_override.items() if v is not None})
                except TypeError: pass
            actual_cell.alignment = final_alignment
            
            final_fill = header_background_fill_to_apply
            if cell_fill_override and isinstance(cell_fill_override, dict):
                params_fill = {k: v for k, v in cell_fill_override.items() if v is not None}
                if "start_color" in params_fill and "fgColor" not in params_fill: params_fill["fgColor"] = params_fill.pop("start_color")
                if "end_color" in params_fill and "bgColor" not in params_fill: params_fill["bgColor"] = params_fill.pop("end_color")
                if "fill_type" in params_fill and "patternType" not in params_fill: params_fill["patternType"] = params_fill.pop("fill_type")
                if params_fill.get("fgColor") and params_fill.get("patternType"):
                    try: final_fill = PatternFill(**params_fill)
                    except TypeError: pass
            if final_fill:
                actual_cell.fill = final_fill
            
            actual_cell.border = header_border_to_apply

            if cell_id:
                if cell_id not in final_column_map:
                    final_column_map[cell_id] = physical_col_idx
                # else:
                    # print(f"Warning: Duplicate header id '{cell_id}' found. Using first occurrence at column {final_column_map[cell_id]}.")

            if rowspan > 1 or colspan > 1:
                end_merge_row = current_ws_row + rowspan - 1
                end_merge_col = physical_col_idx + colspan - 1
                
                # Boundary checks
                if end_merge_row > worksheet.max_row: end_merge_row = worksheet.max_row # Should not happen if rows pre-inserted
                if end_merge_col > max_logical_cols: end_merge_col = max_logical_cols # Cap at max header width

                if end_merge_row >= current_ws_row and end_merge_col >= physical_col_idx:
                    try:
                        worksheet.merge_cells(start_row=current_ws_row, start_column=physical_col_idx,
                                              end_row=end_merge_row, end_column=end_merge_col)
                        # Re-apply style to anchor cell of merge
                        anchor_cell = worksheet.cell(row=current_ws_row, column=physical_col_idx)
                        anchor_cell.alignment = final_alignment
                        anchor_cell.font = final_font
                        if final_fill: anchor_cell.fill = final_fill
                        anchor_cell.border = header_border_to_apply
                    except Exception: # Typically ValueError if cells are already part of another merge
                        # print(f"Debug: Failed to merge R{current_ws_row}C{physical_col_idx} to R{end_merge_row}C{end_merge_col} for value '{value}'. Error: {e_merge_val_err}")
                        pass
            
            # Mark occupied cells in the grid based on physical placement and span
            for r_sp_idx in range(rowspan):
                for c_sp_idx in range(colspan):
                    occ_grid_r = r_offset + r_sp_idx
                    occ_grid_c = (physical_col_idx - 1) + c_sp_idx # physical_col_idx is 1-based
                    if occ_grid_r < num_header_rows and occ_grid_c < max_logical_cols:
                        occupancy_grid[occ_grid_r][occ_grid_c] = True
            
            physical_col_idx += colspan # Advance physical column cursor by the colspan of the cell just processed

    # --- Determine the actual number of physical columns used in the header ---
    actual_physical_cols_used = 0
    for r_idx_occ_check in range(num_header_rows):
        row_max_col_used = 0
        for c_idx_occ_check in range(max_logical_cols - 1, -1, -1): # Iterate backwards
            if occupancy_grid[r_idx_occ_check][c_idx_occ_check]:
                row_max_col_used = c_idx_occ_check + 1 # 1-based
                break
        actual_physical_cols_used = max(actual_physical_cols_used, row_max_col_used)

    if not final_column_map:
        # print("Warning: write_header_new resulted in an empty column_map.")
        # Fallback: try to create a basic map from the first row's values if no IDs were provided
        # This fallback might be less reliable with complex colspans/rowspans if IDs are missing.
        if header_cell_configs and header_cell_configs[0]:
            col_idx_fb = 1
            for cell_cfg_fb_item in header_cell_configs[0]:
                if cell_cfg_fb_item and isinstance(cell_cfg_fb_item, dict):
                    val_str_fb = str(cell_cfg_fb_item.get("value", "")).strip()
                    cell_id_fb = cell_cfg_fb_item.get("id") # Prefer ID if present even in fallback
                    
                    key_to_use_fb = cell_id_fb if cell_id_fb else val_str_fb

                    if key_to_use_fb and key_to_use_fb not in final_column_map:
                        final_column_map[key_to_use_fb] = col_idx_fb
                    
                    col_idx_fb += cell_cfg_fb_item.get("colspan", 1)
                elif cell_cfg_fb_item is None: # Placeholder
                    col_idx_fb +=1 # Assumes colspan 1 for placeholders in this fallback
                if col_idx_fb > max_logical_cols: break
    ...
    return {
        ...
        'num_columns': actual_physical_cols_used
    }
